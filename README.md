...
At this point the site will be running locally at http://localhost:3000/, but the database is brand new and doesn't have any authenticated users, meaning that the majority of the website (the administrative side of things) is completely inaccessible. In order to set up the first administrative user, refer to <a href="#create-a-user">the API documentation</a>.

Note that the administrator page doesn't have a login button on the homescreen, since it's meant to be hidden from general users. To access the page, you have to manually navigate to <a href="http://localhost:3000/login">the login page</a>.

## Source Code Overview

The source code is pretty straightforward to read if you're familiar with the NextJS framework. The only nonstandard organization might be the `/handlers` folder, which is explained thoroughly <a href="api-docs">here</a>. If you're not familiar with NextJS, I would recommend learning a bit about it before going through this source code. However, if you're really adverse to reading documentation, you could just watch <a href="https://youtu.be/__mSgDEOyv8">this</a>. At the time of writing this, NextJS is currently on the verge of releasing a new major version which will introduce the `app` directory as an alternative to `pages`, which means that if you're reading this in the future, this project's structure will be different from modern NextJS projects.

### Database

The <a href="#database">database</a> is also pretty straightforward if you're familiar with the <a href="https://www.prisma.io/">Prisma ORM</a>. Prisma is pretty easy to grasp, and even if you're unfamiliar with it you should be able to infer what's going on with the queries if you have any experience with SQL.

The client-side code doesn't have its own section of documentation here because it's really just React components routed by NextJS's file-based routing system. There's not much   more too it, and any components that *aren't* in `/pages` will be in `/components`.

## Types

Various interfaces are used throughout the application. They are all defined in `/types`, and most of them are based on the autogenerated Prisma types and Zod objects. I made a point of only using interfaces without methods, because I wanted to make the types easy to serialize (which makes database insertion and client-server transmissions much simpler).

<a href="https://zod.dev/">Zod</a> objects are used throughout the program, but primarily to parse the objects sent between the client and the server. If you're unfamiliar with Zod, just know that it's a lightweight library for runtime type-checking. Every single interface in `/types` has it's own Zod object defined in the same file.

# <span id="api-docs">Rest API Documentation</span>

Below is the documentation for the Restful API that was made for this website. The code for this API is found in 

- `/handlers`, where the requests are written, and 
- `/pages/api`, where the handlers are set by NextJS's file-based routing. 

The file for each endpoint is mapped to an appropriate directory in `/handlers`. E.g., to find the handler for a GET request to `/api/exhibit`, look in `/handlers/exhibit/get.ts`. 

Each directory maps to an endpoint in this manner, and the `index.ts` of that directory is where each individual handler function is aggregated into a single `NextApiHandler`. The `index.ts` file is also where middleware is applied to endpoints; middleware is implemented as wrapper functions for `NextApiHandler` functions.
...
### <span id="create-a-user">Create a User</span>
...
To view the structure of the database, refer to the `/prisma/schema.prisma` file. If you are unfamiliar with the Prisma ORM, you can read the documentation <a href="https://www.prisma.io/">here</a>.

## Images

The database doesn't store *all* of the dynamic data the website stores. The API also saves images to the SSD, specifically in `/public/uploads`. NextJS automatically maps the files found in `/public` to GET-able endpoints, which makes it convenient to use images after they've been uploaded. Whenever an image is uploaded or deleted, this is also represented in the `Image` table of the databases which provides the server with a faster way of reading the images than manually checking the filesystem.
